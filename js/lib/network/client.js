...
export class ControlPlaneClient {
    ...
    sendEvent(event, actionStr, commandId = "", cause = "", startTime = 0) {
        ...
        if (event === "death")
            msgType = "BOT_DEATH";
        if (event === "panic_retreat_start")
            msgType = "PANIC_TRIGGERED";
        if (event === "panic_retreat_end")
            msgType = "PANIC_RESOLVED";
        let status = event;
        if (event === "task_completed")
            status = "COMPLETED";
        if (event === "task_failed")
            status = "FAILED";
        if (event === "task_aborted")
            status = "ABORTED";
