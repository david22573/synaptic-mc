...
function getDirectionLabel(lookDir, targetDir, dot) {
    if (dot > 0.8)
        return "center";
    if (dot < -0.2)
        return "behind";
...
let lastPOIUpdate = 0;
let cachedPOIs = [];
export function getPOIs(bot, radius = 32) {
    const now = Date.now();
    if (now - lastPOIUpdate < 1000)
        return cachedPOIs;
    const pois = [];
    if (!bot.entity)
        return pois;
    ...
    for (const id in bot.entities) {
        ...
        if (e.type === "hostile")
            type = "threat";
        else if (e.type === "object" || e.type === "orb")
            type = "resource";
        const baseScore = 100 / Math.max(dist, 1);
        const scoreMultiplier = visibility > 0.5 ? 1.5 : visibility < 0 ? 0.5 : 1.0;
