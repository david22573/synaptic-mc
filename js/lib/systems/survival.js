...
export class SurvivalSystem {
    ...
    async checkSurvival() {
        if (!this.bot?.entity)
            return;
        if (this.diggingEscape || this.pillaringOut)
            return;
        const threats = getThreats(this.bot);
        const immediateThreats = threats.filter((t) => t.distance < 24 &&
            t.threatScore > 5 &&
        ...
        if (immediateThreats.length > 0) {
            ...
            const hasWeapon = this.bot.inventory
                .items()
                .some((i) => i.name.includes("axe") || i.name.includes("sword"));
            const isOneOnOne = immediateThreats.length === 1;
            const isUnavoidableDeath = topThreat.name === "creeper" || topThreat.name === "warden";
            if (hasWeapon &&
                isOneOnOne &&
                this.bot.health > 10 &&
                !isUnavoidableDeath) {
                if (this.isPanicking) {
                    ...
                    this.client.sendEvent("panic_retreat_end", "evasion_complete", "", "engaging_in_combat", 0);
                }
                return;
            }
            ...
            if (!this.isPanicking) {
                ...
                this.config.stopMovement();
                this.client.sendEvent("panic_retreat_start", "evasion", "", topThreat.name, 0);
            }
            if (!this.bot.pathfinder.isMoving() ||
                Date.now() - this.lastFleeTime > 5000) {
                this.lastFleeTime = Date.now();
                const safePos = computeSafeRetreat(this.bot, immediateThreats, 32);
                this.fleeTo(safePos);
            }
        }
        else if (this.isPanicking && Date.now() > this.lastDangerAt + 5000) {
            this.isPanicking = false;
            log.debug("Survival goal reached; securing exit and releasing reflex lock");
            this.config.stopMovement();
            this.pillaringOut = true;
            await this.pillarOut().catch((err) => log.warn("Pillar out failed", { err }));
            this.pillaringOut = false;
            this.client.sendEvent("panic_retreat_end", "evasion_complete", "", "safe", 0);
        }
    }
    async fleeTo(safePos) {
        try {
            const goal = new goals.GoalNearXZ(safePos.x, safePos.z, 2);
            const path = this.bot.pathfinder.getPathTo(this.bot.pathfinder.movements, goal);
            if (path.status === "noPath" || path.status === "timeout") {
                log.warn("Panic pathfinding failed, attempting vertical dig escape");
            ...
            log.warn("Pathfinder threw during panic, attempting vertical dig escape");
            await this.digEscape();
        }
    }
    async digEscape() {
        if (this.diggingEscape)
            return;
        const inWater = this.bot.blockAt(this.bot.entity.position.floored())?.name ===
            "water";
        if (inWater) {
        ...
        try {
            for (let i = 0; i < 3; i++) {
                ...
                if (below &&
                    ...
                    if (tool)
                        await this.bot.equip(tool.type, "hand");
            ...
            if (trashBlock) {
                ...
                for (const vec of compassVectors) {
                    const wallBlock = this.bot.blockAt(pos.offset(vec.x, 2, vec.z));
                    if (wallBlock && wallBlock.boundingBox === "block") {
                        try {
                            await this.bot.placeBlock(wallBlock, vec.scaled(-1));
                            placed = true;
                            log.debug("Successfully sealed roof at surface level.");
                            break;
                        }
                        catch (e) { }
                    }
                ...
                if (!placed) {
                    log.warn("Failed to seal roof. Terrain geometry unsupported.");
                }
            }
        }
        catch (err) {
