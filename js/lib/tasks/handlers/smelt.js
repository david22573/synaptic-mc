...
export async function handleSmelt(ctx) {
    ...
    if (!acceptedInputs) {
        throw new Error(`UNKNOWN_RECIPE: Don't know how to smelt into ${targetName}`);
    }
    const inputItem = bot.inventory
        .items()
        .find((item) => acceptedInputs.includes(item.name));
    if (!inputItem || inputItem.count < targetCount) {
        throw new Error(`MISSING_INGREDIENTS: Need at least ${targetCount} of ${acceptedInputs.join(" or ")}`);
    }
    const fuelItem = bot.inventory
        .items()
        .find((item) => FUEL_TYPES.includes(item.name));
    if (!fuelItem) {
        throw new Error("MISSING_INGREDIENTS: No valid fuel source in inventory");
    }
    ...
    if (!furnaceBlock) {
        ...
        if (!furnaceInInv) {
            throw new Error("NO_FURNACE: No furnace nearby and none in inventory");
        }
        ...
        try {
            await bot.placeBlock(refBlock, bot.entity.position.offset(0, 1, 0).minus(refBlock.position));
            placedFurnace = true;
        }
        catch (err) {
            throw new Error(`PATH_FAILED: Could not place furnace - ${err.message}`);
        }
    ...
    if (bot.entity.position.distanceTo(furnaceBlock.position) > 3) {
        bot.pathfinder.setGoal(new goals.GoalBlock(furnaceBlock.position.x, furnaceBlock.position.y, furnaceBlock.position.z), true);
    ...
    try {
        while (collectedCount < targetCount) {
            ...
            if (!furnace.fuelItem() && furnace.fuel && furnace.fuel === 0) {
                const currentFuel = bot.inventory
                    ...
                    .find((item) => FUEL_TYPES.includes(item.name));
                if (!currentFuel)
                    throw new Error("MISSING_INGREDIENTS: Ran out of fuel during smelting");
            ...
            if (!furnace.inputItem()) {
                const currentInput = bot.inventory
                    ...
                    .find((item) => acceptedInputs.includes(item.name));
                if (!currentInput)
                    throw new Error("MISSING_INGREDIENTS: Ran out of raw materials during smelting");
                const amountToPut = Math.min(currentInput.count, targetCount - collectedCount);
            ...
            if (output && output.name === targetName) {
                ...
                if (taken) {
                    ...
                    log.info(`Smelted ${taken.count} ${targetName}. Total: ${collectedCount}/${targetCount}`);
                }
            }
        ...
        if (placedFurnace && !signal.aborted) {
            const pickType = bot.registry.itemsByName.wooden_pickaxe?.id ||
