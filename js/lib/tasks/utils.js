...
export function isOnSolidGround(bot) {
...
export function isInFoliage(bot) {
    const pos = bot.entity.position.floored();
    const at = bot.blockAt(pos);
    const below = bot.blockAt(pos.offset(0, -1, 0));
    return ((at && TREE_BLOCKS.has(at.name)) ||
        (below && TREE_BLOCKS.has(below.name)));
}
export async function escapeTree(bot, signal) {
    if (!isInFoliage(bot))
        return;
    try {
    ...
    if (!isOnSolidGround(bot)) {
        throw new Error("EscapeTree - could not reach ground after 30 attempts");
    }
}
export function waitForMs(ms, signal) {
...
export function moveToGoal(bot, goal, opts) {
    const { signal, timeoutMs, stopMovement, dynamic = false, stuckTimeoutMs = 5000, } = opts;
    return new Promise((resolve, reject) => {
        if (signal.aborted) {
            reject(new Error("aborted"));
            return;
        ...
        const cleanup = () => {
            listeners.forEach((handler, event) => bot.removeListener(event, handler));
            listeners.clear();
            if (stuckTimer)
...
export function findNearestBlockByName(bot, blockName) {
    return bot.findBlock({
        maxDistance: 32,
        matching: (block) => block?.name === blockName,
    });
}
export async function makeRoomInInventory(bot, slotsNeeded = 1) {
...
export async function placePortableUtility(bot, blockName) {
