...
const { goals } = pkg;
export async function runTask(bot, rawIntent, signal, timeouts, getThreats, computeSafeRetreat, stopMovement) {
    ...
    switch (intent.action) {
        ...
        case "eat": {
            ...
            if (!food)
                throw new Error(`NO_FOOD: ${intent.target.name}`);
            try {
                await bot.equip(food.type, "hand");
                await bot.consume();
            }
            catch (err) {
                throw new Error(`CONSUME_FAILED: ${err instanceof Error ? err.message : String(err)}`);
            }
            return;
        }
        case "idle":
        ...
        case "sleep": {
            ...
            await moveToGoal(bot, new goals.GoalNear(bed.position.x, bed.position.y, bed.position.z, 1.5), { signal, timeoutMs: 20000, stopMovement });
            let onWake;
            let onAbort;
            try {
