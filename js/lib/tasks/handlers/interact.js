...
const { goals } = pkg;
export async function handleInteract(ctx) {
    ...
    if (entity) {
        await moveToGoal(bot, new goals.GoalNear(entity.position.x, entity.position.y, entity.position.z, 2), {
            signal,
    ...
    if (block) {
        await moveToGoal(bot, new goals.GoalNear(block.position.x, block.position.y, block.position.z, 2), {
            signal,
            timeoutMs: timeouts.interact ?? 15000,
            stopMovement,
        });
