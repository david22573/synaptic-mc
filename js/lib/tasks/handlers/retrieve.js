...
class WithdrawState {
    ...
    async execute(ctx) {
        ...
        try {
            ...
            const targetItems = items.filter((i) => i.name === sCtx.targetName);
...
export async function handleRetrieve(ctx) {
    const { bot, intent, signal, timeouts, stopMovement } = ctx;
    await escapeTree(bot, signal);
    const targetName = intent.target?.name;
    if (!targetName) {
        throw new Error("MISSING_INGREDIENTS: No target specified for retrieve");
    }
    const fsmCtx = {
        bot,
