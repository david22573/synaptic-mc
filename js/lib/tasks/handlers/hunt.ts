...
class EngageState implements FSMState {
    ...
    async execute(ctx: StateContext): Promise<FSMState | null> {
        ...
        try {
            while (target && target.isValid && (target.health === undefined || target.health > 0)) {
                ...
                if (Date.now() - combatStartTime > maxCombatDurationMs) {
                    throw new Error("PANIC: Combat loop exceeded max duration.");
                }

                if (Date.now() - sCtx.lastHitTime > 10000) {
                    const isActuallyDead = !bot.entities[target.id];
                    if (isActuallyDead) break;
                    else throw new Error("TARGET_LOST: Failed to damage target for 10s.");
                }

                if (Date.now() - lastValidation > 1000) {
                    const distToTarget = bot.entity.position.distanceTo(target.position);
                    if (distToTarget > 48) throw new Error("TARGET_LOST: Target moved too far away.");
                    ...
                    if (!revalidate || revalidate.id !== target.id) throw new Error("TARGET_LOST: Entity lost.");
                ...
                if (dist <= 3.5) {
                    // Requirement 1: Prevent lookAt if pathfinder is active to resolve orientation contention
                    ...
                    if (now - sCtx.lastAttackTime >= cooldown) {
                        ...
                        // Kiting logic: Backpedal OR strafe while weapon is on cooldown
                        if (dist < 2.5) {
                            // Requirement 3: Normalize diagonal movement vectors by selecting a single axis
                            // to prevent bounding box collisions and physical recoil.
                            bot.clearControlStates();
                            if (Math.random() > 0.5) {
            ...
            if (err.message !== "aborted" && !err.message.includes("TARGET_LOST")) {
                throw new Error(`PANIC: Internal error during combat loop: ${err.message}`);
            }
        ...
        bot.pathfinder.setGoal(new goals.GoalNear(deathPos.x, deathPos.y, deathPos.z, 1));
    ...
    private async ensureBestEquipment(bot: any, sCtx: CombatContext) {
        ...
        if (bestWeapon && (!currentWeapon || currentWeapon.type !== bestWeapon.type)) {
            try { await bot.equip(bestWeapon.type, "hand"); } catch (e) {}
        }
        ...
        if (!offhand || offhand.name !== "shield") {
            ...
            if (shield) {
                try {
                    ...
                    sCtx.hasShield = true;
                } catch (e) { sCtx.hasShield = false; }
            } else { sCtx.hasShield = false; }
        } else { sCtx.hasShield = true; }
    }
}
...
class SearchState implements FSMState {
    ...
    async execute(ctx: StateContext): Promise<FSMState | null> {
        ...
        while (attempts < maxAttempts) {
            ...
            const pos = bot.entity.position.clone().offset(Math.cos(angle) * radius, 0, Math.sin(angle) * radius);
            try {
                bot.pathfinder.setGoal(new goals.GoalNearXZ(pos.x, pos.z, 4));
                for (let i = 0; i < 60; i++) {
                    if (sCtx.signal.aborted) throw new Error("aborted");
                    if (i % 10 === 0 && bot.nearestEntity(filter)) break;
                    await bot.waitForTicks(1);
                }
            } catch (e) { await bot.waitForTicks(10); }
        }
        bot.pathfinder.setGoal(null);
        sCtx.result = { status: "FAILED", reason: `NO_ENTITY: Could not find any ${sCtx.targetName} nearby.` };
        return null;
    }
}
...
export async function handleHunt(ctx: TaskContext): Promise<void> {
    ...
    if (bot.health < 10) throw new Error("SURVIVAL: Health too low to start hunting");
    const targetName = intent.target?.name;
    if (!targetName) throw new Error("MISSING_INGREDIENTS: No target specified.");
