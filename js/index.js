...
const isIgnorableError = (err) => {
    ...
    return (name.includes("PartialReadError") ||
...
console.error = (...args) => {
    ...
    const msg = typeof firstArg === "object"
...
let survival;
let runtimeConfig;
let lastDeathMessage = "unknown causes";
let lastStateSig = "";
...
function completeTask(task, status = "task_completed", cause = "") {
    if (!task || !client)
        return;
    if (currentTask?.id === task.id)
        currentTask = null;
    client.sendEvent(status, `${task.action} ${task.target?.name || "none"}`, task.id, cause, task.startTime);
}
function abortActiveTask(reason) {
...
function normalizeFailureCause(err) {
    ...
    if (msg.includes("no_pickaxe_equipped") || msg.includes("no_tool"))
        return "NO_TOOL";
    if (msg.includes("no_furnace_available") ||
        msg.includes("failed_to_reach_furnace") ||
        msg.includes("no_chest"))
        return "NO_FURNACE"; // Generic missing block error
    if (msg.includes("no_mature_"))
        return "NO_MATURE_CROP";
    if (msg.includes("missing_ingredients") ||
...
async function executeIntent(intent) {
    ...
    if (survival?.isPanickingNow()) {
        if (intent.action === "retreat") {
            ...
            client.sendEvent("task_aborted", "lock", intent.id, "panic", Date.now());
            return;
        }
    }
    ...
    try {
        ...
        await Promise.race([
            runTask(bot, intent, signal, timeouts, () => getThreats(bot), (t) => computeSafeRetreat(bot, t, 20), stopMovement),
            new Promise((_, r) => {
...
function pushState() {
    ...
    const now = Date.now();
    if (sig === lastStateSig && now - lastStatePushTime < 5000)
        return;
    lastStateSig = sig;
...
async function connectWithRetry(maxAttempts = 10) {
    ...
    bot.on("windowOpen", (window) => {
        if (String(window.type).includes("chest") ||
