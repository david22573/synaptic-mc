...
export function getThreats(bot) {
    const threats = [];
    const botPos = bot.entity.position;
    for (const id in bot.entities) {
        const e = bot.entities[id];
        if (!e || !e.isValid || e === bot.entity)
            continue;
        ...
        if (effectiveDistance > 16)
            continue;
        const baseThreat = config.THREAT_WEIGHTS[e.name?.toLowerCase() || ""] || 5;
...
export function computeSafeRetreat(bot, threats, distance = 20) {
    ...
    for (const angle of testAngles) {
        ...
        for (let yOffset = 2; yOffset >= -5; yOffset--) {
            const block = bot.blockAt(botPos.offset(Math.cos(angle) * distance, yOffset, Math.sin(angle) * distance));
            ...
            if (block && block.boundingBox === "block") {
                ...
                const above = bot.blockAt(botPos.offset(Math.cos(angle) * distance, yOffset + 1, Math.sin(angle) * distance));
                if (above &&
