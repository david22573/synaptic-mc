...
const DATA_DIR = path.resolve(__dirname, "../node_modules/minecraft-assets/minecraft-assets/data");
const DEST_DIR = path.resolve(__dirname, "../public/assets/items");
function findBestVersionFolder() {
    ...
    if (fuzzyMatches.length > 0) {
        console.log(`[i] Exact match for ${TARGET_VERSION} not found. Using closest available: ${fuzzyMatches[0]}`);
        return path.join(DATA_DIR, fuzzyMatches[0]);
    }
    // 3. Complete failure
    console.error(`[!] Fatal: Could not find any assets for ${TARGET_VERSION}.x`);
    console.error(`[i] Available versions in package: ${folders.join(", ")}`);
    return null;
}
function extractAssets() {
    console.log(`[+] Starting asset extraction for Minecraft ${TARGET_VERSION} branch...`);
    ...
    copyFiles(blockDir);
    console.log(`[+] Hell yeah. Successfully ripped ${copied} textures into ${DEST_DIR}`);
}
extractAssets();
//# sourceMappingURL=extract_assets.js.map